...
class LoadedMission: protected BasicMission
{
    ...
    std::array<unsigned, ORDERS_SIZE> killed;
...
public:
    ...
    LoadedMission(const BasicMission & quest, const Stats & st, const std::string & missionGetter): BasicMission(quest)
    {
		...
		unsigned missionid = rand()%20000+1;
        BasicMission::getFileName() = PLAYER_DIR+st.getName()+"\\"+BasicMission::getFileName()+std::to_string(missionid);
        who = missionGetter;
        save();
    }
	...
	bool isTargetKilled(const Stats & st);
