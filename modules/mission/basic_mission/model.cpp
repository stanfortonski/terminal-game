...
array<string, ORDERS_SIZE> BasicMission::getOrderedItems() const
{
    ...
    {
        string temp;
        BasicInterpreter::load("must_item["+to_string(i+1)+"]", temp);
...
array<pair<string, unsigned>, ORDERS_SIZE> BasicMission::getOrderedMurders() const
{
    ...
    for (unsigned i = 0; i < ORDERS_SIZE; ++i)
    {
        ...
        BasicInterpreter::load("ordermurder_name["+to_string(i+1)+"]", name);
        BasicInterpreter::load("ordermurder_amount["+to_string(i+1)+"]", amount);
...
void BasicMission::setOrderedItems(const array<string, ORDERS_SIZE> & arr) const
{
    ...
    {
        if (!arr[i].empty())
            BasicInterpreter::save("must_item["+to_string(i+1)+"]", arr[i]);
    }
}
...
void BasicMission::setOrderedMurders(const array<pair<string, unsigned>, ORDERS_SIZE> & arr) const
{
    for (unsigned i = 0; i < ORDERS_SIZE; ++i)
    {
        ...
        {
            BasicInterpreter::save("ordermurder_name["+to_string(i+1)+"]", arr[i].first);
            BasicInterpreter::save("ordermurder_amount["+to_string(i+1)+"]", arr[i].second);
        }
    }
}
