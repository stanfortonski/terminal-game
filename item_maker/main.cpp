...
int main()
{
    for (;;)
    {
        ...
        cout<<"Podaj wymaganą siłe do noszenia: ";
        need_str = setNumber<unsigned>();

        cout<<"Podaj wymaganą zręczność do noszenia: ";
        ...
        {
            cout<<"Podaj instancje przedmiotu którego nauczysz się wykonać: ";
            getline(cin, bufor[0]);
            bufor[0] = bufor[0];
            cin.clear();
        }
        ...
        {
            string temp;
            cout<<"Podaj instancje potrzebnego składnika by wykonac ten przedmiot (Uwaga max 10, q - nie ustawiaj składników): ";
            for (unsigned i = 1; getline(cin, temp) && temp != "q" && i < bufor.max_size(); ++i)
            {
                ...
                if (i+1 != bufor.max_size())
                    cout<<i+1<<". Podaj instancje potrzebnego składnika (q - zakończy dodawanie składników): ";
            }
            cin.clear();
            while (cin.get() != '\n') continue;
        }
        ...
        cout<<"Podaj ile przywraca życia: ";
        it.getHp() = setNumber<long>();
        it.save();

        cout<<"Podaj ile dodaje maksymalnego życia: ";
    	it.getMaxHp() = setNumber<long>();
        it.save();

        cout<<"Podaj regeneracje życia: ";
        ...
        cout<<"Podaj obrażenia minimalne: ";
        it.getMinDmg() = setNumber<int>();
        it.save();

        cout<<"Podaj obrażenia maksymalne: ";
        ...
        cout<<"Podaj zręczność: ";
        it.getDex() = setNumber<double>();
        it.save();

        screen::clear();
        it.show();
        cout<<"Ukończono tworzenie przedmiotu!\n\n[Dalej]";
        screen::paused();
    }
    cout<<"Gotowe!\n";
}
